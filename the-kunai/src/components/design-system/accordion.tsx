import {
    ...
    AccordionItem,
    AccordionTrigger,
} from "@/components/ui/accordion" // Based on Radix or similar
...
function getAccordionVariantClasses(variant: string) {
    ...
    switch (variant) {
        case "bottom-border":
            variantClass = "!border-b !border-[#E5E5E5] hover:underline";
            break;
        case "bordered":
    ...
    return (
        ...
        >
            <AccordionItem value={itemValue} disabled={disabled}>
                <AccordionTrigger
                    className={cn("w-full transition-all flex items-center [&_svg]:text-[#767676]",
                    ...
                    {...triggerProps}
                >
                    <div className={cn("flex hover:no-underline tracking-wide",  titleClassName)}>
                        {
                            triggerPosition !== 'left' && titleIcon && <span className="mr-2">{titleIcon}</span>
                        }
                        <span>{title}</span>
                    </div>
                </AccordionTrigger>
